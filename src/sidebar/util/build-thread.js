 ...
 * @prop {(a: Thread, b: Thread) => number} [replySortCompareFn] - comparison
 *       function for sorting replies
 */

/**
 * Default top-level thread sorting function if none provided via options.
 *
 * Sort by local application-applied `$tag`
 *
 * @param {Thread} a
 * @param {Thread} b
 * @return {number}
 */
const defaultSortCompareFn = (a, b) => {
  if (a.annotation && b.annotation) {
    if (a.annotation.$tag < b.annotation.$tag) {
      return -1;
    } else if (a.annotation.$tag > b.annotation.$tag) {
      return 1;
    }
  }
  return 0;
...
/**
 * Default reply thread sorting function if none provided via options.
 *
 ...
 * @return {number}
 */
const defaultReplySortCompareFn = (a, b) => {
  if (!a.annotation || !b.annotation) {
    return 0;
  }
...
export default function buildThread(annotations, options) {
  ...
  const replyCompareFn =
    options.replySortCompareFn ?? defaultReplySortCompareFn;
  thread = sortThread(thread, compareFn, replyCompareFn);
